...
The API load balancer offers an experimental routing mode that builds on top of the existing funtionality. It is now possible to use your load balancer endpoint for all Supabase services (Auth, Edge Functions, Realtime, and Storage) meaning you do not need to worry about which endpoint you need to use in which situations.
...
The experimental routing mode can be enabled by sending the header `sb-lb-routing-mode: alpha-all-services` along with any requests. This gives the option to opt-in and try out the new routing, but still have the peace of mind that the original behavior is there should it be needed. Please let us know if you have any feedback by [contacting us](https://supabase.com/support).

What follows is an example of using the `supabase-js` library to create a new client with the appropriate headers.

```ts ./create-client.ts
import { createClient } from "@supabase/supabase-js";

// Supabase credentials from the environment
...
This client creation function can be used throughout an application as required to take advantage of the new experimental routing functionality.
